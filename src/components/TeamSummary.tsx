...
const TeamSummary: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async (): Promise<void> => {
      try {
        ...
        setError(
          `Failed to load team data: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        setLoading(false);
  ...
  const getFormResults = () => {
    ...
    return [0, 1, 2, 3, 4].map((i) => {
      ...
      return {
        ...
        color:
          result === "1"
            ? "bg-score-win-primary"
            : result === "0"
              ? "bg-score-draw-primary"
              : "bg-score-loss-primary",
        colorText:
          result === "1"
            ? "text-score-win-foreground"
            : result === "0"
              ? "text-score-draw-foreground"
              : "text-score-loss-foreground",
      };
    });
  };
  ...
  const chartData = [
    {
      category: "wins",
      percentage: parseInt(data.wins || "0"),
      fill: "var(--color-wins)",
    },
    {
      category: "losses",
      percentage: parseInt(data.losses || "0"),
      fill: "var(--color-losses)",
    },
    {
      category: "draws",
      percentage: parseInt(data.ties || "0"),
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <StatCard title="Team Overview" description="General team statistics">
          <div className="space-y-2">
            ...
            <StatRow
              label="Best Division"
              value={`Division ${data.bestDivision}`}
            />
            <StatRow
              label="Best Finish"
              value={`Group ${data.bestFinishGroup}`}
            />
          </div>
        </StatCard>
          ...
          footer={
            <div className="flex justify-between w-full">
              ...
              <span className="font-bold text-score-win-primary">
                {(
                  (parseInt(data.wins) / parseInt(data.gamesPlayed)) *
                  100
                ).toFixed(1)}
                %
              </span>
        ...
        <StatCard title="Current Form" description="Recent performance">
          <div className="space-y-4">
            <StatRow label="Win Streak" value={data.wstreak} />
            <StatRow label="Unbeaten Streak" value={data.unbeatenstreak} />
            <div className="mt-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <StatCard title="Goals" description="Scoring statistics">
          <div className="space-y-2">
            ...
            <StatRow
              ...
              win={
                parseInt(data.goals || "0") -
                  parseInt(data.goalsAgainst || "0") >
                0
              }
              loss={
                parseInt(data.goals || "0") -
            ...
            <StatRow
              ...
              value={
                parseInt(data.gamesPlayed) > 0
                  ? (parseInt(data.goals) / parseInt(data.gamesPlayed)).toFixed(
                      2,
                    )
                  : "0.00"
              }
        ...
        <StatCard
          ...
          description="Promotion and relegation history"
        >
          <div className="space-y-2">
            <StatRow
