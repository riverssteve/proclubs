  ...
  if (error || !data) return (
    <div className="flex flex-col items-center justify-center h-screen">
      <div className="text-red-500 text-xl mb-4">
        <span className="font-bold">Error:</span> {error || "Failed to load player data"}
      </div>
      <div className="max-w-md text-center text-gray-700">
        <p>Unable to connect to the data API. Please check your connection and try again later.</p>
        <p className="mt-2 text-sm text-gray-500">API endpoint: /api/proclubs?platform=common-gen5&clubId={CLUB_ID}</p>
      </div>
    </div>
  );
